...
?>
  ...
  <?php
    ...
    try {
        $productSql = "SELECT ProductName FROM products WHERE ProductID = :productID";
        $productStmt = $pdo->prepare($productSql);
        $productStmt->execute([':productID' => $productID]);
    
        // Kiểm tra sản phẩm có tồn tại không
        if ($productStmt->rowCount() > 0) {
            $productName = $productStmt->fetch(PDO::FETCH_ASSOC)['ProductName'];
        } else {
    ...
    // Truy vấn dữ liệu từ bảng feedback
    try {
        $sql = "SELECT FeedbackID, UserID, Message, Date FROM feedback WHERE ProductID = :productID ORDER BY Date DESC";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([':productID' => $productID]);
    
        // Hiển thị feedback
        echo "<h2>Feedback for $productName</h2>";
        if ($stmt->rowCount() > 0) {
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                echo "<div class='feedback'>";
                echo "<div class='feedback-id'><strong>Feedback ID:</strong> " . htmlspecialchars($row['FeedbackID']) . "</div>";
                echo "<div class='user-id'><strong>User ID:</strong> " . htmlspecialchars($row['UserID']) . "</div>";
                echo "<div class='message'><strong>Message:</strong> " . htmlspecialchars($row['Message']) . "</div>";
                echo "<div class='date'><strong>Date:</strong> " . htmlspecialchars(date("Y-m-d H:i:s", strtotime($row['Date']))) . "</div>";
                echo "</div><hr>";
            }
        } else {
